...
function Hamburger({ }) {
    ...
    return (
        <>
            <button className={`flex flex-col justify-between items-baseline p-0 cursor-pointer w-[30px] h-6 transition-all ease-in-out after:transition-all after:ease-in-out z-40 relative ${open ? 'translate-x-44' : 'translate-x-0'}`} onClick={handleClickOpen}>
            ...
            <div className={`bg-black absolute top-0 left-0 z-20 h-full w-full ${open ? 'opacity-50' : 'opacity-0'}`}></div>

            <div className={`bg-white absolute h-full top-0 left-0 w-[300px] z-30 rounded-r-[30px] transition-transform overflow-hidden ${open ? 'translate-x-0' : '-translate-x-[300px]'}`}>
